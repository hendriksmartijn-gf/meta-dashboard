  ...
  return (
    <div className="bg-white border border-[#E2DBFF] p-6">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-6">
        <h2 className="font-semibold text-[#0f0f0f] text-base leading-tight pr-4">{campaignName}</h2>
        <div className="flex flex-wrap gap-1 shrink-0">
          {METRICS.map((metric) => (
            <button
              key={metric.key as string}
              onClick={() => toggleMetric(metric.key as string)}
              className={`px-2.5 py-1 text-[10px] font-semibold uppercase tracking-widest transition-colors border ${
                activeMetrics.has(metric.key as string)
                  ? 'text-white border-transparent'
                  : 'bg-white text-slate-400 border-[#E2DBFF] hover:border-[#A38DFB]'
              }`}
              style={
                activeMetrics.has(metric.key as string)
